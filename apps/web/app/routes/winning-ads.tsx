      ...
      {ads.length === 0 ? (
        ...
        <Card>
          ...
          <div className="divide-y divide-outline-variant">
            {ads.map((a, idx) => (
              <Link key={a.id} to={`/item/ad/${a.id}`} className="flex items-center gap-4 px-container-padding py-4 hover:bg-surface-dim/30 transition-colors">
                <span className="font-metric-md text-metric-md tabular-nums text-on-surface-variant w-8 text-center">{idx + 1}</span>
                <MediaImage src={a.imageUrl} seed={a.id + (a.copy ?? "")} format={a.format} className="w-16 h-16 rounded flex-shrink-0" />
